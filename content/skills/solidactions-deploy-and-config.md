...
- **`.env` and any `.env.*` are ALWAYS excluded and cannot be re-included** — secrets reach the sandbox via `solidactions env set` (→ `ctx.vars`), never the bundle. (Only exact `.env` / `.env.*` basenames match; `.envrc` and `environment.ts` are unaffected.)
...
# Pull resolved variables to a local .env file (a local snapshot for reference/backup — `solidactions dev` does not read it):
...
solidactions env set my-project SENDGRID_API_KEY "sk-test-..." -e dev
solidactions dev src/hello.ts -e dev
#    ↳ ctx.vars.SENDGRID_API_KEY is populated from the dev environment

# Option B: skip the platform — pass values directly as workflow input:
solidactions dev src/hello.ts -i '{"apiKey": "sk-test-..."}'
```

*Why: the platform will not hand out secret plaintext to a resolve call, and a local `.env` would compete with the `ctx.vars` model (an AI writing workflows would reach for `process.env` instead). So local dev either fetches non-secret vars from a real environment you've configured, or takes values explicitly as input — never from a file or the host shell.*
...
There is one schedule per workflow. If one already exists, `schedule set` updates it; it does not add a second schedule. As of CLI v3.6, every `schedule` subcommand takes `-e, --env <environment>`, so target a specific environment explicitly rather than relying on the default — run `solidactions schedule <subcommand> --help` to confirm on your version. (Before v3.6 these commands had no environment flag at all, which is why older guidance says to use the app when a project family has multiple environments.)
