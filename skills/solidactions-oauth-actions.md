...
When a tenant connects a third-party service through the SolidActions UI, the workflow runtime resolves each `oauth:`-mapped var as a **`ConnectionVar`** object available at `ctx.vars.<NAME>` inside the `defineWorkflow` run body. `SA_PROXY_URL` and `SA_PROXY_TOKEN` are **reserved infra keys** consumed by the SDK — they are NOT readable via `ctx.vars` or `process.env` in your code.

A `ConnectionVar` bundles everything you need to call the proxy:

```typescript
import { defineWorkflow, type ConnectionVar } from '@solidactions/sdk';

export const workflow = defineWorkflow({
  name: 'my-workflow',
  run: (ctx) => {
    const conn = ctx.vars.GMAIL as ConnectionVar;
    // conn.proxyUrl   — base URL of the proxy for this run
    // conn.proxyToken — short-lived bearer token, rotated per run
    // conn.key        — connection key string identifying the provider account
  },
});
```
...
```
At runtime, `ctx.vars.PERSONAL_GMAIL` and `ctx.vars.WORK_GMAIL` are each a `ConnectionVar` with their own `proxyUrl`, `proxyToken`, and `key`.

A call to a third-party API is **always** a `fetch` against the proxy:

```
${conn.proxyUrl}/<platform-slug>/<path-from-catalog>
```
...
- **Do not omit `connectionKey` from a modifier action's body just because it feels like a credential.** When `inputSchema.body.required` lists `connectionKey`, include it. The value is `conn.key` — the same string you put in the `X-OAuth-Connection-Key` header.
- **Do not put your connection key in the `Authorization` header.** That's where the run token (`conn.proxyToken`) goes; the connection key goes in `X-OAuth-Connection-Key` (and, for modifiers, also in the body).
...
The string on the right (`"Gmail (production)"`) is the connection name the tenant typed when creating the connection in the SolidActions UI. The local name (`GMAIL`) becomes `ctx.vars.GMAIL` inside your workflow — it is a `ConnectionVar` whose `key`, `proxyUrl`, and `proxyToken` are resolved per-run by the platform.
...
export const workflow = defineWorkflow({
  ...
  run: async (ctx) => {
    ...
    const res = await fetch(
      `${conn.proxyUrl}/gmail/gmail/v1/users/${userId}/messages/send`,
      {
        method: 'POST',
        headers: {
...
The URL is mechanically `${conn.proxyUrl}/<platform-slug>${path}` where `path` comes verbatim from the catalog. For Gmail's `path: /gmail/v1/users/{{userId}}/messages/send` that produces a double `gmail/gmail/v1/...` — that's expected (the platform slug and the upstream path both happen to start with `gmail`). For other providers it won't double up.
...
export const workflow = defineWorkflow({
  ...
  run: async (ctx) => {
    const conn = ctx.vars.GOOGLE_CALENDAR as ConnectionVar;
    ...
    const res = await fetch(
      `${conn.proxyUrl}/google-calendar/calendars/${calendarId}/events?${params}`,
      {
        method: 'GET',
        headers: {
          'Authorization':          `Bearer ${conn.proxyToken}`,
...
- **Custom modifier**: path uses a non-RESTful slug (e.g. `POST /gmail/get-emails`, `POST /slack/send-message`). Body schema includes a `connectionKey` field listed in `inputSchema.body.required`. **You include `connectionKey: conn.key` in the body.** Modifier actions often resolve N upstream calls into one, returning fully-hydrated data instead of stub IDs.
...
The URL shape and required headers are identical for both flavors. The only difference is whether the body has a `connectionKey` field. In both cases `conn` is `ctx.vars.GMAIL as ConnectionVar`:

```ts
// Raw upstream — path: /gmail/v1/users/{{userId}}/messages/send
fetch(`${conn.proxyUrl}/gmail/gmail/v1/users/${userId}/messages/send`, {
  method: 'POST',
  headers: {
    'Authorization':          `Bearer ${conn.proxyToken}`,
    'X-OAuth-Connection-Key': conn.key,
...
fetch(`${conn.proxyUrl}/gmail/gmail/get-emails`, {
  method: 'POST',
  headers: {
    'Authorization':          `Bearer ${conn.proxyToken}`,
    'X-OAuth-Connection-Key': conn.key,
  ...
  body: JSON.stringify({
    connectionKey: conn.key,  // ← required for modifiers
    query: 'is:unread',
    maxResults: 10,
  }),
});
...
Both URLs read as `${conn.proxyUrl}/<platform-slug>${path}`. The double-slug (`gmail/gmail/...`) is a coincidence of Gmail's catalog `path` field starting with `/gmail/`; for Google Calendar (`path: /calendars/...`) the URL is `${conn.proxyUrl}/google-calendar/calendars/...` — no double-slug.

## Common Mistakes

- **Importing a provider SDK.** `import { google } from 'googleapis'` will fail at runtime — there is no provider token to give it. Always `fetch` against `conn.proxyUrl`.
- **Reading proxy coordinates from `process.env`.** `process.env.SA_PROXY_URL` and `process.env.SA_PROXY_TOKEN` are reserved infra keys consumed by the SDK — they are NOT available in your workflow code. Use `conn.proxyUrl` and `conn.proxyToken` from the `ConnectionVar` at `ctx.vars.<NAME>`.
- **Hardcoding a fallback proxy URL.** `conn.proxyUrl || 'https://app.solidactions.com/api/v1/proxy'` defeats local-dev: in local-dev runs the proxy URL points at a tunnel, and the fallback hides misconfiguration. Let the `ConnectionVar` fail loudly if missing.
- **Sending the connection key as a Bearer token.** `Authorization: Bearer ${conn.key}` is wrong — `conn.key` goes in `X-OAuth-Connection-Key`; `conn.proxyToken` goes in `Authorization`.
...
- **Forgetting `connectionKey` in a modifier body when `inputSchema.body.required` lists it.** The proxy does not auto-inject this field. Look at the schema; if it's required, put `conn.key` into the body.
...
- **Reading `ctx.vars.GMAIL_CONNECTION_KEY` instead of `ctx.vars.GMAIL`.** The runtime resolves under exactly the env name you wrote in `solidactions.yaml` — no suffix, no transformation. If your yaml has `GMAIL: { oauth: ... }`, the var is `ctx.vars.GMAIL`.
